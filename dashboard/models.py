...
def license_plate_image_upload_path(instance, filename):
    return os.path.join('license_plate_images', filename)

class LprAlpha(Base):
    camera = models.ForeignKey(Camera, on_delete=models.PROTECT, related_name="lpralpha")
    plate_number = models.CharField(max_length=20)
    confidence_score = models.FloatField(validators=[MinValueValidator(0), MaxValueValidator(1)])
    lpr_image = models.ImageField(upload_to=license_plate_image_upload_path)
    car_image = models.ImageField(upload_to=car_image_upload_path)
